...
int runCameraCalibration(CvCapture* capture,
	...
	sprintf(calibDir, "%s" SLASH_STRING "calib" SLASH_STRING "cam", sl_params->outdir);
	sprintf(str, "%s" SLASH_STRING "calib", sl_params->outdir);
	_mkdir(str);
	_mkdir(calibDir);
#if !defined(__linux__)
	...
	while(successes < n_boards){
		...
		cvShowImageResampled((char*)"camWindow", frame, sl_params->window_w, sl_params->window_h);
	...
	if(successes >= 2){
		...
		for(int i=0; i<successes; ++i){
			sprintf(str,"%s" SLASH_STRING "%02d.png", calibDir, i);
			cvSaveImage(str, calibImages[i]);
			cvGetRow(rotation_vectors, r, i);
			cvRodrigues2(r, R, NULL);
			sprintf(str,"%s" SLASH_STRING "cam_rotation_matrix_%02d.xml", calibDir, i);
			//cvSave(str, R);
		}
		sprintf(str,"%s" SLASH_STRING "cam_intrinsic.xml", calibDir);	
		cvSave(str, sl_calib->cam_intrinsic);
		sprintf(str,"%s" SLASH_STRING "cam_distortion.xml", calibDir);
		cvSave(str, sl_calib->cam_distortion);
		sprintf(str,"%s" SLASH_STRING "cam_rotation_vectors.xml", calibDir);
		cvSave(str, rotation_vectors);
		sprintf(str,"%s" SLASH_STRING "cam_translation_vectors.xml", calibDir);
...
int runProjectorCalibration(CvCapture* capture, 
	...
	if(successes >= 2){
		...
		for(int i=0; i<successes; ++i){
			...
			sprintf(str,"%s" SLASH_STRING "%02db.png", calibDir, i);
			cvSaveImage(str, cam_calibImages[i]);
			cvGetRow(proj_rotation_vectors, r, i);
			cvRodrigues2(r, R, NULL);
			sprintf(str,"%s" SLASH_STRING "proj_rotation_matrix_%02d.xml", calibDir, i);
			//cvSave(str, R);
		}
		sprintf(str,"%s" SLASH_STRING "proj_intrinsic.xml", calibDir);	
		cvSave(str, sl_calib->proj_intrinsic);
		sprintf(str,"%s" SLASH_STRING "proj_distortion.xml", calibDir);
		cvSave(str, sl_calib->proj_distortion);
		sprintf(str,"%s" SLASH_STRING "proj_rotation_vectors.xml", calibDir);
		cvSave(str, proj_rotation_vectors);
		sprintf(str,"%s" SLASH_STRING "proj_translation_vectors.xml", calibDir);
